 ...
 *  to override.
 */
public class DMASTFactory extends CoreASTFactory {
     ...
     *
     *  Useless since internal tree nodes are already created in DMParserDefinition.
     */
    @NotNull
    @Override
    ...
    public LeafElement createLeaf(@NotNull IElementType type, CharSequence text) {
        if ( type instanceof TokenIElementType &&
                ((TokenIElementType) type).getANTLRTokenType()==DMIndentingLexer.NAME)
        {
            ...
            return new NamePsiNode(type, text);
        }
