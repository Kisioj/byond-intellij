...
public abstract class DMReference extends PsiReferenceBase<NamePsiNode> {
    public DMReference(@NotNull NamePsiNode element) {
        /** WARNING: You must send up the text range or you get this error:
         * "Cannot find manipulator for PsiElement(NAME) in org.antlr.jetbrains.sample.DMElementRef"...
...
//		System.out.println(getClass().getSimpleName()+".isReferenceTo("+refName+"->"+def.getText()+")");
        ...
        if ( def instanceof NamePsiNode && isDefSubtree(def.getParent()) ) {
            def = def.getParent();
        }
        if ( isDefSubtree(def) ) {
